/*********************************************************************
 *
 * $Id: yocto_voc.cpp 10245 2013-03-11 13:35:54Z seb $
 *
...
const string YVoc::LOGICALNAME_INVALID = "!INVALID!";
const string YVoc::ADVERTISEDVALUE_INVALID = "!INVALID!";
const string YVoc::UNIT_INVALID = "!INVALID!";
const double YVoc::CURRENTVALUE_INVALID = -DBL_MAX;
const double YVoc::LOWESTVALUE_INVALID = -DBL_MAX;
const double YVoc::HIGHESTVALUE_INVALID = -DBL_MAX;
const double YVoc::CURRENTRAWVALUE_INVALID = -DBL_MAX;
const double YVoc::RESOLUTION_INVALID = -DBL_MAX;
const string YVoc::CALIBRATIONPARAM_INVALID = "!INVALID!";

std::map<string,YVoc*> YVoc::_VocCache;

int YVoc::_parse(yJsonStateMachine& j)
{
 ...
 */
string YVoc::get_logicalName(void)
{
 ...
 */
int YVoc::set_logicalName(const string& newval)
{
    string rest_val;
    rest_val = newval;
 ...
 */
string YVoc::get_advertisedValue(void)
{
    if(_cacheExpiration <= YAPI::GetTickCount()) {
 ...
 */
double YVoc::get_currentValue(void)
{
    if(_cacheExpiration <= YAPI::GetTickCount()) {
 ...
 * On failure, throws an exception or returns a negative error code.
 */
int YVoc::set_lowestValue(double newval)
{
    string rest_val;
 ...
 */
double YVoc::get_lowestValue(void)
{
    if(_cacheExpiration <= YAPI::GetTickCount()) {
 ...
 * On failure, throws an exception or returns a negative error code.
 */
int YVoc::set_highestValue(double newval)
{
    string rest_val;
    char buf[32]; sprintf(buf,"%d", (int)floor(newval*65536.0 +0.5)); rest_val = string(buf);
 ...
 */
double YVoc::get_highestValue(void)
{
    if(_cacheExpiration <= YAPI::GetTickCount()) {
 ...
 */
double YVoc::get_currentRawValue(void)
{
    if(_cacheExpiration <= YAPI::GetTickCount()) {
 ...
 */
double YVoc::get_resolution(void)
{
...
string YVoc::get_calibrationParam(void)
{
    if(_cacheExpiration <= YAPI::GetTickCount()) {
...
int YVoc::set_calibrationParam(const string& newval)
{
    string rest_val;
    rest_val = newval;
 ...
 * On failure, throws an exception or returns a negative error code.
 */
int YVoc::calibrateFromPoints(vector<double> rawValues,vector<double> refValues)
{
    string rest_val;
...
int YVoc::loadCalibrationPoints(vector<double> rawValues,vector<double> refValues)
{
    if(_cacheExpiration <= YAPI::GetTickCount()) {
...
void YVoc::registerValueCallback(YVocUpdateCallback callback)
{
    if (callback != NULL) {
...
void YVoc::advertiseValue(const string& value)
{
    if (_callback != NULL) {
...
YVoc* YVoc::FindVoc(const string& func)
{
    if(YVoc::_VocCache.find(func) != YVoc::_VocCache.end())
        return YVoc::_VocCache[func];
    
    YVoc *newVoc = new YVoc(func);
    YVoc::_VocCache[func] = newVoc;
