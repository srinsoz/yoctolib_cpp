/*********************************************************************
 *
 * $Id: yocto_voltage.cpp 10245 2013-03-11 13:35:54Z seb $
 *
...
const string YVoltage::LOGICALNAME_INVALID = "!INVALID!";
const string YVoltage::ADVERTISEDVALUE_INVALID = "!INVALID!";
const string YVoltage::UNIT_INVALID = "!INVALID!";
const double YVoltage::CURRENTVALUE_INVALID = -DBL_MAX;
const double YVoltage::LOWESTVALUE_INVALID = -DBL_MAX;
const double YVoltage::HIGHESTVALUE_INVALID = -DBL_MAX;
const double YVoltage::CURRENTRAWVALUE_INVALID = -DBL_MAX;
const double YVoltage::RESOLUTION_INVALID = -DBL_MAX;
const string YVoltage::CALIBRATIONPARAM_INVALID = "!INVALID!";

std::map<string,YVoltage*> YVoltage::_VoltageCache;

int YVoltage::_parse(yJsonStateMachine& j)
{
    ...
    while(yJsonParse(&j) == YJSON_PARSE_AVAIL && j.st == YJSON_PARSE_MEMBNAME) {
        if(!strcmp(j.token, "logicalName")) {
            ...
            if(yJsonParse(&j) != YJSON_PARSE_AVAIL) return -1;
            _currentValue =  floor(atof(j.token)/655.36+.5) / 100;
        } else if(!strcmp(j.token, "lowestValue")) {
            if(yJsonParse(&j) != YJSON_PARSE_AVAIL) return -1;
            _lowestValue =  floor(atof(j.token)/655.36+.5) / 100;
        } else if(!strcmp(j.token, "highestValue")) {
            if(yJsonParse(&j) != YJSON_PARSE_AVAIL) return -1;
            _highestValue =  floor(atof(j.token)/655.36+.5) / 100;
 ...
 */
string YVoltage::get_logicalName(void)
{
 ...
 */
int YVoltage::set_logicalName(const string& newval)
{
    string rest_val;
    rest_val = newval;
 ...
 */
string YVoltage::get_advertisedValue(void)
{
    if(_cacheExpiration <= YAPI::GetTickCount()) {
 ...
 */
double YVoltage::get_currentValue(void)
{
    if(_cacheExpiration <= YAPI::GetTickCount()) {
 ...
 * On failure, throws an exception or returns a negative error code.
 */
int YVoltage::set_lowestValue(double newval)
{
    string rest_val;
 ...
 */
double YVoltage::get_lowestValue(void)
{
    if(_cacheExpiration <= YAPI::GetTickCount()) {
 ...
 * On failure, throws an exception or returns a negative error code.
 */
int YVoltage::set_highestValue(double newval)
{
    string rest_val;
    char buf[32]; sprintf(buf,"%d", (int)floor(newval*65536.0 +0.5)); rest_val = string(buf);
 ...
 */
double YVoltage::get_highestValue(void)
{
    if(_cacheExpiration <= YAPI::GetTickCount()) {
 ...
 */
double YVoltage::get_currentRawValue(void)
{
    if(_cacheExpiration <= YAPI::GetTickCount()) {
 ...
 */
double YVoltage::get_resolution(void)
{
...
string YVoltage::get_calibrationParam(void)
{
    if(_cacheExpiration <= YAPI::GetTickCount()) {
...
int YVoltage::set_calibrationParam(const string& newval)
{
    string rest_val;
    rest_val = newval;
 ...
 * On failure, throws an exception or returns a negative error code.
 */
int YVoltage::calibrateFromPoints(vector<double> rawValues,vector<double> refValues)
{
    string rest_val;
...
int YVoltage::loadCalibrationPoints(vector<double> rawValues,vector<double> refValues)
{
    if(_cacheExpiration <= YAPI::GetTickCount()) {
...
YVoltage *YVoltage::nextVoltage(void)
{
    ...
    return yFindVoltage(hwid);
}

void YVoltage::registerValueCallback(YVoltageUpdateCallback callback)
{
    if (callback != NULL) {
...
void YVoltage::advertiseValue(const string& value)
{
    if (_callback != NULL) {
...
YVoltage* YVoltage::FindVoltage(const string& func)
{
    if(YVoltage::_VoltageCache.find(func) != YVoltage::_VoltageCache.end())
        return YVoltage::_VoltageCache[func];
    
    YVoltage *newVoltage = new YVoltage(func);
    YVoltage::_VoltageCache[func] = newVoltage;
